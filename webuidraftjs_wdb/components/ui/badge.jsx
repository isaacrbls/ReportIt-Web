...
export function RecentReports({ onVerify, onReject, onViewDetails, statusFilter }) {
	console.log("RecentReports mounted");
	...
	useEffect(() => {
		const fetchReports = async () => {
			...
			try {
				...
				console.log('All fetched reports:', reportsData.map(r => ({ id: r.id, Status: r.Status })));
				if (statusFilter && statusFilter !== "All Reports") {
					const normalizedStatus = statusFilter.trim().toLowerCase();
					console.log('Normalized status for filter:', normalizedStatus);
					const filtered = reportsData.filter(r => (r.Status || '').trim().toLowerCase() === normalizedStatus);
